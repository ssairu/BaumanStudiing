...
void setup() {
  ...
  server.on("/post", HTTP_GET, [](AsyncWebServerRequest *request){
    String message;
    request->send(200, "text/html", "<form method=\"post\" enctype=\"multipart/form-data\"><div><label for=\"file\">Choose file to upload</label><input type=\"file\" id=\"file\" name=\"file\" multiple /></div><div><button>Submit</button></div></form>");
        if (request->hasParam("file", true)) {
            message = request->getParam("file", true)->value();
